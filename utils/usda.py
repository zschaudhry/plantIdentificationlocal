...
def format_usda_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in invasive_df.columns:
        ...
        if pd.api.types.is_object_dtype(col_dtype):
            ...
            if (
                sample.str.match(r"^\d{4}-\d{2}-\d{2}T").any()
                or sample.str.match(r"^\d{4}-\d{2}-\d{2}$").any()
            ):
                invasive_df[col] = pd.to_datetime(
                    invasive_df[col], errors="coerce"
                ).dt.strftime("%Y-%m-%d")
            elif (
                sample.str.match(r"^\d{12,}").any()
                or sample.str.match(r"^\d{10,}").any()
            ):
        ...
        elif pd.api.types.is_integer_dtype(col_dtype) or pd.api.types.is_float_dtype(
            col_dtype
        ):
            sample = invasive_df[col].dropna().astype(str).head(10)
            if (
    ...
    if "LAST_UPDATE" in invasive_df.columns:
        try:
            invasive_df["LAST_UPDATE_sort"] = pd.to_datetime(
                invasive_df["LAST_UPDATE"], errors="coerce"
            )
            invasive_df = invasive_df.sort_values(
                "LAST_UPDATE_sort", ascending=False
            ).drop(columns=["LAST_UPDATE_sort"])
        except Exception:
            pass
    return invasive_df
